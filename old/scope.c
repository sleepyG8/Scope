...
for (int i = 0; i < nt->FileHeader.NumberOfSections; i++, section++) {
    ...
    if (strcmp(section->Name, "PAGE") == 0 || strcmp(section->Name, ".text") == 0) {
       ...
       for (int i=0; i < textSize; i++) {
            ...
            printf("\x1b[32m0x%02X %02X %02X %02X %02X at [%d] \x1b[0m", textStart[i], textStart[i + 1], textStart[i + 3], textStart[i + 4], textStart[i + 5], i);
            continue;
            }
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 2) {
        printf("Usage: Scanner.exe <\\SystemRoot\\system32\\ntoskrnl.exe> (Must use this format)");
        return 1;
    }
    ...
    //getting the names from the buffer
        ...
        if (strcmp(argv[1], (char*)modules->Modules[i].FullPathName) == 0) {
        char path[100];
